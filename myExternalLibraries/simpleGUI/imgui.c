...
/*
 * 由于simpleGUI对于颜色的支持使人一言难尽，且C语言不存在类/类继承/函数重载的概念，
 * 我不得不把写的函数复制下来进行更改。
 * 将函数名前面的 my 去掉即为原函数。如有需求，可直接与原函数diff，从而得知改动细节。
 * 免责声明：原函数与新函数的的版权归均simpleGUI原作者所有，本作者不对新函数声明版权。
 */
...
static int MyMenuItem(int id, double x, double y, double w, double h, char* label)
{
	...
	mySetPenColor(frameColor);
	drawBox(x, y, w, h, 0, label, 'L', labelColor);
...
int MyMenuList(int id, double x, double y, double w, double wlist, double h, char* labels[], int n)
{
	...
	if (MyMenuItem(id, x, y, w, h, labels[0]))
		unfoldMenu = !unfoldMenu;
	...
	if (gs_UIState.actingMenu == id && unfoldMenu) {
		...
		setMenuRect(x, x + wlist, y - n * h + h, y);
		for (k = 1; k < n; k++) {
			if (MyMenuItem(GenUIID(k) + id, x, y - k * h, wlist, h, labels[k])) {
				unfoldMenu = 0;
				setMenuRect(0, 0, 0, 0);
				result = k;
			}
